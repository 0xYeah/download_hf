...
```shell
go build -o download_mf main.go
```

或使用构建脚本（生成带版本信息的压缩包）：

```shell
bash build.sh          # release 模式（默认）
...
./download_mf -p Jackrong/Qwopus3.5-27B-v3-GGUF

# 指定保存路径
./download_mf --output /data/models Jackrong/Qwopus3.5-27B-v3-GGUF
./download_mf -o /data/models Jackrong/Qwopus3.5-27B-v3-GGUF

# 后台下载
./download_mf --daemon Jackrong/Qwopus3.5-27B-v3-GGUF
./download_mf -d Jackrong/Qwopus3.5-27B-v3-GGUF

# 组合：国内镜像 + 指定路径 + 后台运行
./download_mf -p -o /data/models -d Jackrong/Qwopus3.5-27B-v3-GGUF
```
